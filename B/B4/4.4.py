...
class LastSeenLog:
    ...
    def find(self, user_id):
        ...
        if user_id in self.log:
            # если такой пользователь есть в журнале, получаем время его последней активности
            last_seen_time_stamp = self.log[user_id]
            # конвертируем полученный timestamp в объект datetime, используя библиотеку datetime
            last_seen_date_time = datetime.fromtimestamp(last_seen_time_stamp)
